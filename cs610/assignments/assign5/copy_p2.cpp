...
void push(uint32_t v) {
    ...
    //cout << "after creation: new node created successfully -> " << v << endl;

    //load value of current top of stack 
    /*head_node = put_count_bits(head_node);*/
    uint64_t old_head = head_node.load();
    uint64_t new_head= put_count_bits(temp);

    //next of new node to be old head of stack with counter bits
...
int pop(){
   ...
   // delete remove_count_bits(old_head);
    return poped_value;
}
    ...
    for (int j = 0; j < runs; j++) {
        ...
        for(int i=0; i<NUM_OPS; i++){
            ...
            if(i >= ADD){
                ...
                #pragma omp critical
                {
                    cout << "Thread #" << thread_id << " is going to pop " << endl;
                }
                */
                popped_value = pop();
            }
                ...
                /*#pragma omp critical
                {
                    cout << "Thread #" << thread_id << " is going to add " << insert_value << endl;
                }
                */
                push(insert_value);
            }
            ...
            /*#pragma omp critical*/
            /*{*/
            /*    string job = (i < ADD)? "PUSH" : "POP";*/
            /*    int32_t value = (i < ADD)? insert_value:popped_value;*/
            /*    cout << "Thread #" << thread_id << " " << job << "ed " << value << endl;*/
            /**/
            /*}*/
        }
        end = HR::now();
        ...
        while(start && start->next_node){
            Node* temp = start->next_node;
            delete start;
            start = temp;
        }
