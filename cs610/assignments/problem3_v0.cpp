...
int main(int argc, char* argv[])
{
    ...
    if(debug)
        cout << "Starting the job here" << endl;
...
#pragma omp critical (read_from_file)
        {
            ...
            if(debug)
                cout << "thread-id: " << tid << ", completed reading" << endl;
...
#pragma omp critical (write_to_buffer)
        {
            if(debug)
                cout << "thread-id: " << tid << ", is here to write in buffer" << endl;
            ...
            while(!thread_buff.empty())
            {
                ...
                if(debug)
                    cout << "thread-id: " << tid << ", took buffer lock" << endl;
                ...
                if(buffer.size() == MAX_BUFFER_SIZE){
                    ...
                    if(debug)
                        cout << "thread-id: " << tid << ", signalled to consumer" << endl;

                    while(run_consumer) //waiting for consumer's signal
                        pthread_cond_wait(&ack_cond, &buffer_lock);

                    if(debug)
                        cout << "thread-id: " << tid << ", received signal from consumer" << endl;
                    ...
                    if(debug)
                        cout << "thread-id: " << tid << ", still has " << thread_buff.size() << " lines to write" << endl;
                }

                pthread_mutex_unlock(&buffer_lock);
                if(debug)
                    cout << "thread-id: " << tid << ", released buffer lock" << endl;
            }
     ...
     ************************************************/
        if(debug)
            cout << "thread-id: " << tid << ", has entered the consumer arena" << endl;
        while(READING)
        {
            pthread_mutex_lock(&buffer_lock);
            if(debug)
            ...
            */
            ...
            if(debug)
                cout << "thread-id: " << tid << ", consumer signalled prod" << endl;
            ...
            if(debug)
                cout << "thread-id: " << tid << ", consumer received signal" << endl;
            ...
            pthread_mutex_unlock(&buffer_lock);
            if(debug)
                cout << "thread-id: " << tid << ", consumer released buffer_lock" << endl;
        }
        ...
        if(debug)
            cout << "consumer cleaning the last ones" << endl;
...
//    {
//        if(debug)
//            cout << "thread-id: " << tid <<", doing the final end-job" << endl;
//        READING = false;
//    }
} //paralle-region ends here

    if(debug)
        cout << "Did we ever reach here?" << endl;
}
